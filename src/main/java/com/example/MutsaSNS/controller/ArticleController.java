...
public class ArticleController {
    ...
    // 피드 작성 API
    @PostMapping()
    public ResponseDto createArticle(HttpServletRequest request, @RequestBody ArticleDto articleDto) {
        ...
        try {
            articleService.createArticle(username, articleDto);
    ...
    public ResponseDto readFollowingArticle(
        ...
        try {
            ...
            if (articleDtos.size() == 0) responseDto.getResponse().put("message", "작성된 게시글이 없습니다");
                ...
                responseDto.getResponse().put("result", articleDtos);
                responseDto.getResponse().put("message", "팔로잉 중인 사람들의 게시글 목록을 불러오는 데 성공했습니다");
            }
        } catch (RuntimeException error) {
            responseDto.getResponse().put("error", error.getMessage());
        }
